...
class sst5_trainer:
    ...
    def _train(self):
        ...
        for batch in tqdm(self.train_loader, desc='training...'):
            ...
            onehot_label=F.one_hot(label,self.opt.polarities_dim[self.opt.dataset]).to(self.device)
            
            self.optimizer.zero_grad()
            prediction,loss = self.model(input_ids,token_type_ids,attention_mask,onehot_label)
            
            
            pred.extend((torch.argmax(prediction,dim=-1)).int().cpu().numpy().flatten().tolist())
        ...
        return np.mean(epoch_losses), accuracy,micro_f1,macro_f1
    ...
    def _test(self):
        ...
        return np.mean(epoch_losses), accuracy,micro_f1,macro_f1,pred,labels
