...
class bert_MAAML(base_bert):
    def __init__(self,opt):
        super().__init__(opt)
        self.adms_loss = MultiAngularPenaltySMLoss(36,self.opt.polarities_dim[opt.dataset], loss_type='arcface',s=opt.s,m=opt.m,em_num_of_perclass=opt.em_num_of_perclass,inner_margin=opt.inner_margin).to(self.device)
